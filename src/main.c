...
int main(int argc, char** argv) {
    uint64_t thread_counts[] = {1, 2, 4, 8, 16, 32, 64};
    uint64_t cat_counts[] = {100, 1000, 10000, 100000, 1000000, 10000000};
    ...
    for (size_t i = 0; i < thread_count_len; i++) {
        for (size_t j = 0; j < cat_count_len; j++) {
            char command[100];
            snprintf(command, sizeof(command), "monte_carlo.exe %llu %llu", thread_counts[i], cat_counts[j]);
